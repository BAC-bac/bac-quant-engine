...
def summarise_results(results: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        results.groupby(["pair", "signal_v5", "direction", "horizon", "horizon_bars"])
        .agg(
...
def main() -> None:
    ...
    if not all_results:
        print("\nNo D1 v5 backtest results generated.")
        return
    ...
    print(f"\nSaved detailed D1 v5 results to: {OUTPUT_FILE}")
    print(f"Saved summary D1 v5 results to:  {SUMMARY_FILE}")

    print("\nD1 v5 summary preview:")
