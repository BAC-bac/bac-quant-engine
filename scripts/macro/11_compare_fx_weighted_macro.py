...
INPUT_FILE = PROJECT_ROOT / "macro_data" / "processed" / "sovereign_weighted_scores.csv"
OUTPUT_FILE = PROJECT_ROOT / "macro_data" / "processed" / "fx_weighted_macro_bias.csv"
...
def main() -> None:
    ...
    for pair, (base_country, quote_country) in FX_PAIRS.items():
        ...
        base_score = float(base_row.iloc[0]["weighted_sovereign_score"])
        quote_score = float(quote_row.iloc[0]["weighted_sovereign_score"])
        ...
        results.append(
            {
                ...
                "quote_weighted_score": quote_score,
                "score_diff": diff,
                "macro_bias": classify_bias(diff),
            }
        )
    ...
    print(f"Saved weighted FX macro bias to: {OUTPUT_FILE}")
    print()
    print(f"Weighted macro FX comparison ({latest_year}):")
