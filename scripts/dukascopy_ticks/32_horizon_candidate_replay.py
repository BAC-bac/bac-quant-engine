...
def build_candidate_path(symbol: str) -> Path:
    return (
        ...
        / f"symbol={symbol}"
        / "top_signals"
        / "top_horizon_signal_candidates_latest.csv"
    )
...
def load_candidates(candidate_path: Path) -> pd.DataFrame:
    ...
    required = {"feature", "target", "side", "horizon_signal_rank"}
...
def run_horizon_candidate_replay(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Dukascopy horizon candidate replay."
    )
