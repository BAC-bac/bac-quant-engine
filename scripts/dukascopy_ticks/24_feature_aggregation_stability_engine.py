...
Purpose:
    Aggregate Script 22 feature discovery scores into stable feature rankings.

Refactor note:
    This script can run standalone with --symbol and also exposes
    run_feature_stability() for orchestrators.
"""
...
def run_feature_stability(symbol: str = DEFAULT_SYMBOL, top_n: int = TOP_N) -> Path | None:
    symbol = symbol.upper().strip()
    ...
    for (feature, target), g in df.groupby(["feature", "target"]):
        ...
        dominant_direction = "positive" if positive_rate >= negative_rate else "negative"
    ...
    with open(output_report, "w", encoding="utf-8") as f:
        ...
        if top.empty:
            f.write("No stable features generated.\n")
        else:
            f.write(
                top[
                    [
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Aggregate Dukascopy feature discovery scores into stable rankings."
    )
...
def main() -> None:
    args = parse_args()
    run_feature_stability(symbol=args.symbol, top_n=args.top_n)
