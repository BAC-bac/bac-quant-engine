"""
BACQE MICROSTRUCTURE 36 - REGIME CONDITIONED SIGNALS

Purpose:
    Build and walk-forward validate signals where direction is conditioned
    by liquidity regime.

Core idea:
    Instead of:
        one global signal direction

    Use:
        train fold liquidity regime behaviour
        -> infer long/short/neutral per regime
        -> apply those regime-specific directions to the test fold
"""
...
def infer_regime_signal_map(train_df: pd.DataFrame, target: str) -> dict:
    regime_map = {}
...
def evaluate_regime_conditioned_signal(
...
def run_conditioned_signal_for_candidate(row: pd.Series) -> list[dict]:
    ...
    for fold_idx, (train_start, train_end, test_end) in enumerate(splits, start=1):
        ...
        regime_map = infer_regime_signal_map(train_df, target)

        signal_col = "regime_conditioned_signal"
        test_df[signal_col] = apply_regime_signal_map(test_df, regime_map)

        metrics = evaluate_regime_conditioned_signal(test_df, target, signal_col)
...
def classify_conditioned_signal(row: pd.Series) -> str:
    ...
    if (
        ...
        and avg_hit_rate >= 0.57
        and avg_mean_return > 0
        and positive_fold_rate >= 0.60
    ):
        return "conditioned_signal_strong"
    ...
    if (
        ...
        return "conditioned_signal_research"
    ...
    if (
        ...
        return "conditioned_signal_weak"

    return "no_conditioned_signal_edge"


def summarise_conditioned_signals(fold_df: pd.DataFrame) -> pd.DataFrame:
    ...
    summary["conditioned_signal_score"] = (
    ...
    summary["conditioned_signal_label"] = summary.apply(classify_conditioned_signal, axis=1)
    summary["created_at_utc"] = datetime.now(timezone.utc).isoformat()

    label_rank = {
        "conditioned_signal_strong": 1,
        "conditioned_signal_research": 2,
        "conditioned_signal_weak": 3,
        "no_conditioned_signal_edge": 4,
        "low_sample": 5,
        "insufficient": 6,
    }

    summary["label_rank"] = summary["conditioned_signal_label"].map(label_rank).fillna(99)

    summary = summary.sort_values(
        [
            "label_rank",
            "conditioned_signal_score",
            "avg_hit_rate",
    ...
    summary["conditioned_signal_rank"] = summary.index + 1

    return summary


def main() -> None:
    print_header("BACQE MICROSTRUCTURE 36 - REGIME CONDITIONED SIGNALS")
    ...
    report_dir = analysis_dir / "regime_conditioned_signals"
    ...
    print(f"Conditioned candidates: {len(candidates_df):,}")
    print("-" * 90)

    if candidates_df.empty:
        raise RuntimeError("No signal candidates found for conditioned signal validation.")

    fold_records = []

    for idx, row in candidates_df.iterrows():
        records = run_conditioned_signal_for_candidate(row)
        fold_records.extend(records)

        print(
            f"[COND_SIGNAL] {idx + 1:>2}/{len(candidates_df)} "
            f"{row['symbol']:<8} "
            f"{row['bar_type']:<22} "
            f"{row['parameter']:<26} "
            f"{row['target']:<16} "
    ...
    summary_df = summarise_conditioned_signals(fold_df)

    fold_csv = report_dir / "microstructure_regime_conditioned_signal_folds_latest.csv"
    summary_csv = report_dir / "microstructure_regime_conditioned_signal_summary_latest.csv"
    json_path = report_dir / "microstructure_regime_conditioned_signals_latest.json"
    txt_path = report_dir / "microstructure_regime_conditioned_signals_latest.txt"
    ...
    if not summary_df.empty:
        label_counts = summary_df["conditioned_signal_label"].value_counts(dropna=False).to_dict()
    else:
        label_counts = {}

    display_cols = [
        "conditioned_signal_rank",
        "symbol",
        "bar_type",
        "parameter",
        "spread_feature",
    ...
    lines = []
    lines.append("=" * 90)
    lines.append("BACQE MICROSTRUCTURE REGIME CONDITIONED SIGNALS")
    lines.append("=" * 90)
    ...
    lines.append(f"Conditioned labels:    {label_counts}")
    lines.append("")
    lines.append("-" * 90)
    lines.append("TOP REGIME CONDITIONED SIGNAL RESULTS")
    lines.append("-" * 90)

    if summary_df.empty:
        lines.append("No conditioned signal summary available.")
    else:
    ...
    print("-" * 90)
    print("[DONE] Regime conditioned signal validation complete.")
    print(f"Signal factory rows: {len(signal_df):,}")
    ...
    print(f"Conditioned labels:  {label_counts}")
